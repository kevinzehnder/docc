...
Every key a theme YAML file may set, what it accepts, and what happens when it
is absent. It opens with the narrative — how a theme works — and continues with
the exhaustive key list.
...
```yaml
# themes/_house.yaml — a fragment, extended but never selected
name: _house
...
```yaml
# themes/_house.yaml — a fragment: it exists to be extended
name: _house
